...
Mesh::Mesh(std::vector<Vertex> *vertices, std::vector<unsigned int> *indices,
    ...
    : m_Vertices(vertices), m_Indices(indices), m_Shader(shader),
      ...
      m_ModelMatrix(1.0f), m_TimeCreated(glfwGetTime())
{
}
...
void Mesh::Initialize()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, 2 * m_Vertices->size() * sizeof(Vertex),
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 2 * m_Indices->size() * sizeof(unsigned int), (void *)0,
                 GL_DYNAMIC_DRAW);
...
void Mesh::Update()
{
    ...
    if (m_TimeDelta < 0.25)
    {
        m_ModelMatrix = glm::translate(
            ...
            glm::vec3(0.0f,
                      static_cast<float>(
                          10.0 * glm::sin(2 * 3.141592653589 * m_TimeDelta)),
                      0.0f));

...
Mesh::Mesh(std::vector<Vertex> *vertices, std::vector<unsigned int> *indices,
           Shader *shader)
    : p_Vertices(vertices), p_Indices(indices), m_Shader(shader),
      m_Count(static_cast<unsigned int>(indices->size())), m_PushedToGPU(false),
      m_ModelMatrix(1.0f) {
...
void Mesh::Initialize() {
  ...
  glBufferData(GL_ARRAY_BUFFER, 2 * p_Vertices->size() * sizeof(Vertex),
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               2 * p_Indices->size() * sizeof(unsigned int), (void *)0,
               GL_DYNAMIC_DRAW);
  ...
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                        (void *)(offsetof(Vertex, m_TextureCoordinate)));
...
void Mesh::Update() {
  ...
  if (m_TimeDelta < 0.5) {
    m_ModelMatrix = glm::translate(
        ...
        glm::vec3(
            0.0f,
            static_cast<float>(10.0 * glm::sin(3.141592653589 * m_TimeDelta)),
            0.0f));

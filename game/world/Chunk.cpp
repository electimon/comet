...
void Chunk::GenerateSurface()
{
  ...
  for (int x = 0; x < CHUNK_WIDTH; x++)
  {
    for (int z = 0; z < CHUNK_WIDTH; z++)
    {
      ...
      height = 40.0f + (20.0f * noise1) + (10.0f * noise2) + (5.0f * noise3) +
               (2.5f * noise4);
    ...
    for (int y = 0; y < CHUNK_HEIGHT; y++)
    {
      for (int z = 0; z < CHUNK_WIDTH; z++)
      {
        ...
        if (x == 0)
        {
          ...
          nxBlock = World::GetBlock({x + m_Chunk.x * CHUNK_WIDTH - 1, y,
                                     z + m_Chunk.z * CHUNK_WIDTH});
        }
        else
        {
          if (x == CHUNK_WIDTH - 1)
          {
            pxBlock = World::GetBlock({x + m_Chunk.x * CHUNK_WIDTH + 1, y,
                                       z + m_Chunk.z * CHUNK_WIDTH});
        ...
        if (z == 0)
        {
          ...
          nzBlock = World::GetBlock({x + m_Chunk.x * CHUNK_WIDTH, y,
        ...
        else
        {
          if (z == CHUNK_WIDTH - 1)
          {
            pzBlock = World::GetBlock({x + m_Chunk.x * CHUNK_WIDTH, y,
                                       z + m_Chunk.z * CHUNK_WIDTH + 1});
